...
\begin{flalign*}
  ...
  \int \tan x \,dx 
  &= \int \frac{\sin x}{\cos x} \,dx \quad 分数関数はまず最初に→分母の微分を考える \\
...
\begin{flalign*}
  ...
  \int \frac{dx}{\sin^2 x} 
  &= \int \frac{dx}{\cos^2 \left(\frac{\pi}{2} - x \right)} \\
...
\begin{flalign*}
  \int_{-1}^{1} \sqrt{1-x^2} \,dx \quad \sqrt{(\qquad)^2} 
  &=|\qquad| としたい \\
  ここで,1-\sin^2 t = \cos^2 tである \\
  また,1-x^2 \ge 0 \Leftrightarrow -1 \le x \le 1 であるから, x=\sin t とおける \\
  x 
  &= \sin t \: とする \\
  dx 
  &= \cos t \,dt \\
...
\begin{flalign*}
  ...
  ここで,1+\tan^2 t = \frac{1}{\cos^2t}である \\
  また,(\tan t)' = \frac{1}{\cos^2 t}であるから,x=\tan tとする \\
  %x &= \tan t \: とおく \\
...
\begin{flalign*}
  \int_{0}^{e} \frac{e^x}{e^{e-x}+e^x} \,dx 
  &= \int_{0}^{e} \frac{e^x}{e^{e-x}+e^x} \,dx \\
  \quad a+b-xを含む,下端が0,指数関数の式→KingProperty \\
...
\begin{flalign*}
  \int \frac{dx}{\sin^4 x} \quad f(\sin^2 x,\cos^2 x,\tan x)→g(\tan x) \cdot \frac{1}{\cos^2 x}に \\
  \int \frac{dx}{\sin^4 x} 
  &= \int \frac{dx}{\tan^4 x \cos^4 x} \\
...
\begin{flalign*}
  \int_{0}^{\frac{\pi}{2}} \cos^2 x \,dx & \quad 積分区間の下端が0,三角関数の式で積分区間が{\frac{n\pi}{2}}→KingProperty \\
  I 
...
\begin{flalign*}
  \int \frac{\log \left(\sin x \right)}{\tan x} \,dx \\
  \tan の一次式 
  &→ \frac{\sin}{\cos},真数,位相が複雑 \\
  &→ =tとして微分接触を疑う \\
  \int \frac{\log \left(\sin x \right)}{\tan x} \,dx 
...
\begin{flalign*}
  ...
  &= \int_{0}^{\frac{\pi}{6}} \frac{\frac{\sqrt{3}}{\cos^2 t}}{3\tan^2 t +3} \,dt \\
...
\begin{flalign*}
  \int \frac{dx}{\cos^3 x} 三角関数は2乗に強い,三角関数や指数関数の分数関数 \\
  ...
  &= \left(\frac{1}{2}\left(\frac{1}{1+t} + \frac{1}{1-t}\right) \right)^2 \\
...
\begin{flalign*}
  \int \frac{dx}{\sin^4 x} \quad f(\sin^2 x,\cos^2 x,\tan x)→t=\tan x \\
  t 
  &= \tan x \\
  \cos^2 x 
  &= \frac{1}{1+\tan^2 x} \\
...
\begin{flalign*}
  \int_{0}^{2\pi} \sqrt{1+\cos x} \,dx \quad \sqrt{(\qquad)^2} 
  &= |\qquad| としたい.\\
...
\begin{flalign*}
  ...
  \int_{0}^{\frac{\pi}{3}} \sqrt{1+\sin x} \,dx 
  &= \int_{0}^{\frac{\pi}{3}} \sqrt{1+\cos \left(\frac{\pi}{2}-x\right)} \,dx \\
...
=======
  ...
  &= 0\\
>>>>>>> e3dd2dc8c7bc52b0c157260b74837c6346ae697c
  \frac{1}{2} \int_{0}^{1} \frac{dx}{\left(x-\frac{1}{2}\right)^2+\frac{3}{4}} \\
  x-\frac{1}{2} 
  &= \frac{\sqrt{3}}{2}\tan t \\
  dx 
  &= \frac{\sqrt{3}}{2\cos^2 t} \,dt \\
  \frac{1}{2} \int_{0}^{1} \frac{dx}{\left(x-\frac{1}{2}\right)^2+\frac{3}{4}} 
