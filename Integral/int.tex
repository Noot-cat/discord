...
\begin{flalign*}
  ...
  &また,1-x^2 \ge 0⇔-1 \le x \le 1 であるから, xは x=\sin t と表せる. \\
  x &= \sin t \: とする \\
  dx &= \cos t \,dt \\
...
\begin{flalign*}
  ...
  & ここで,1+\tan^2 t=\frac{1}{\cos^2 t}である. \\
  & また,(\tan t)'=\frac{1}{\cos^2 t}であるから,x=\tan tとしてみる. \\
  x &= \tan t \: とおく \\
...
\begin{flalign*}
  ...
  \int \tan^4 x \,dx &= \int \tan^2 x \tan^2 x \,dx \quad \tan^2 は\cos^2 \\
...
\int_{0}^{e} \frac{e^x}{e^{e-x}+e^x} \,dx &=\int_{0}^{e} \frac{e^x}{e^e \cdot e^{-x}+e^x} \,dx \\
...
\begin{flalign*}
  ...
  \int \frac{dx}{\cos^3 x} \\
  & 三角関数は2乗に強い,三角関数や指数関数の分数関数,分母を優先 \\
